...
@section('title', 'Edit Model | PosSales')

@section('content_header')
<div class="d-flex justify-content-between align-items-center">
    <h1 style="font-size:1.3rem; font-weight:600; color:#343a40;">
        <i class="fas fa-cube mr-2" style="color:#6c757d;"></i> Edit Model
    </h1>
    <div>
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('models.update', $model->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" placeholder="Enter model name" required value="{{ old('name', $model->name) }}">
                                @error('name')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" placeholder="auto-generated-from-name" value="{{ old('slug', $model->slug) }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Enter model description">{{ old('description', $model->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-upload-container" id="imageUploadContainer" style="border: 2px dashed #6c757d; border-radius: 8px; padding: 30px; text-align: center; background: #f8f9fa; cursor: pointer; transition: all 0.3s;">
                            ...
                            <div class="upload-placeholder" id="uploadPlaceholder" style="{{ $model->image ? 'display: none;' : '' }}">
                            ...
                            <div class="image-preview" id="imagePreview" style="{{ $model->image ? 'display: block;' : 'display: none;' }}">
                                @if($model->image)
                                    <img src="{{ asset('storage/' . $model->image) }}?{{ time() }}" alt="{{ $model->name }}" style="max-width: 150px; max-height: 130px; border-radius: 4px; margin-bottom: 10px;">
                                @else
                                    <img src="" alt="Preview" style="max-width: 150px; max-height: 130px; border-radius: 4px; margin-bottom: 10px;">
                                @endif
                                <p class="mb-0">
                                    <span id="fileName">{{ $model->image ? basename($model->image) : '' }}</span> 
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="alert alert-info">
                    ...
                    <ul class="mb-0">
                        <li>Edit model name or slug</li>
                        <li>Change associated brand</li>
                        <li>Upload new image if needed</li>
                        <li>Toggle active status</li>
                    </ul>
                </div>
                <div class="alert alert-warning">
                    <h6><i class="fas fa-exclamation-triangle mr-2"></i> Important</h6>
                    <p class="mb-0">Changes apply immediately to all products using this model.</p>
                </div>
            </div>
        </div>
    </div>
